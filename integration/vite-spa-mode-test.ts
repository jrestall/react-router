...
test.describe("SPA Mode", () => {
  ...
  test.describe("custom builds", () => {
    ...
    test.describe("<HydratedRouter routes> prop", () => {
      ...
      test("Throws an error if users provide duplicate index routes", async ({
        ...
        expect(logs).toEqual([
          expect.stringContaining(
            "Error: Cannot add a duplicate child index route to the root route via the `HydratedRouter` `routes` prop.  The `routes` prop will be ignored."
          ),
        ]);
      });
      ...
      test("Throws an error if users provide duplicate path routes", async ({
        ...
        expect(logs).toEqual([
          expect.stringContaining(
            "Error: Cannot add a duplicate child route with path `/path` to the root route via the `HydratedRouter` `routes` prop.  The `routes` prop will be ignored."
          ),
        ]);
      });
    });
